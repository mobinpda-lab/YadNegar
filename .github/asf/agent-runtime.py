...
def build_context(title, body):
    ...
    for path in run(["git","ls-files"], check=False).stdout.splitlines():
        ...
        low=path.lower(); score=(20 if low in {"readme.md","pubspec.yaml"} else 0)+(8 if low.startswith("docs/") else 0)+(4 if any(t in low for t in terms) else 0)+(3 if any(x in low for x in ("lib/","test/",".github/")) else 0)
...
def main():
    ...
    if not raw:return 2
    issue_number=int(raw); expected=os.getenv("YADNEGAR_EXPECTED_MAIN_SHA") or os.getenv("ASF_EXPECTED_MAIN_SHA") or ""; repo=os.environ["GITHUB_REPOSITORY"]
    ...
    issue=github_json(f"https://api.github.com/repos/{repo}/issues/{issue_number}"); labels={x.get("name") for x in issue.get("labels",[])}
    if not ({"factory:ready","factory:leased"}&labels): raise RuntimeError("Issue is not opted into autonomous execution")
    lease(issue_number); title=issue.get("title",""); body=issue.get("body","") or ""; context=build_context(title,body); deadline=time.monotonic()+PROVIDER_BUDGET; failure=""
    for attempt in range(1,MAX_FIX_ATTEMPTS+1):
        ...
        if passed: print("YADNEGAR_CODE_WORKER_PASS"); return 0
